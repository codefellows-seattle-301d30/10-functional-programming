...
((module) => {
  let Article = function (rawDataObj) {
  ...
  Article.numWordsAll = () => {
    return Article.all.map(articleThing => articleThing.body.split(" ")).reduce((wordCount, wordList) => {
      return wordCount += wordList.length;
    }, 0)
  };

  Article.allAuthors = () => {
    return Article.all.map(art => art.author).reduce((authorList, author) => {
